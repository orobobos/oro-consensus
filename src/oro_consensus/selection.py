...
def compute_selection_weight(candidate: ValidatorCandidate) -> float:
    ...
    - Reputation bonus: up to 1.25x at rep=1.0
    - Attestation bonus: up to 1.3x with 3+ attestations
    - Tenure penalty: 0.9^(tenure-4) after 4 consecutive epochs
    - Performance factor: 0.9-1.1x based on last epoch
    ...
    reputation_factor = 1.0 + (rep_excess * 0.5)  # Up to 1.25x at rep=1.0

    # Attestation bonus
    attestation_count = candidate.valid_attestation_count()
    attestation_factor = 1.0 + (0.1 * min(attestation_count, 3))  # Up to 1.3x
    ...
    if candidate.last_epoch_performance:
        ...
        # Range 0.9-1.1x based on participation rate
...
def _force_new_validators(
    ...
    new_candidates = [
        (c, v) for c, v in all_candidates if c.agent_id not in selected_ids and c.agent_id not in previous_validators
    ]
...
def _compute_candidate_ticket(
    ...
    # Weight of 2.0 gives ~2x better chance
