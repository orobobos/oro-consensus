...
def compute_tenure_penalty(consecutive_epochs: int) -> float:
    ...
    - 10% reduction (0.9x) per epoch after that
...
def _analyze_voting_correlation(
    ...
    high_correlation_pairs = [
        (v1, v2, corr) for (v1, v2), corr in correlation_matrix.items() if corr >= VOTING_CORRELATION_THRESHOLD
    ]
...
def _analyze_federation_clustering(validator_set: ValidatorSet) -> list[CollusionAlert]:
    ...
    for federation, count in federation_counts.items():
        if count > max_allowed:
            ...
            validators_from_fed = [
                v.agent_id for v in validator_set.validators if federation in v.federation_membership
            ]
...
class AntiGamingEngine:
    ...
    def analyze_validator_set(
        ...
        validators_at_penalty = sum(
            1 for v in validator_set.validators if v.tenure_epochs > MAX_CONSECUTIVE_EPOCHS_BEFORE_PENALTY
        )
        ...
        tenure_stats = {
            "validators_at_penalty": validators_at_penalty,
